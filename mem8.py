...
# Add total memory usage like free -h
total_physical_memory = psutil.virtual_memory().available / (1024 ** 2)
total_virtual_memory = sum((proc['memory_usage'] * psutil.cpu_count() for proc in all_processes))
print("\nTotal Memory Usage:")
print(f"{'Free':<15} {psutil.disk_usage('/').free / (1024 ** 3):.2f} GB")
print(f"{'Used':<15} {psutil.disk_usage('/').used / (1024 ** 3):.2f} GB")
print(f"{'Total':<15} {total_physical_memory:.2f} GB")
print("\nTotal Virtual Memory Usage:")
print(f"{'Sum':<15} {total_virtual_memory:.2f} GB")
